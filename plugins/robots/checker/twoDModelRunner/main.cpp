...
int main(int argc, char *argv[])
{
	...
	QCommandLineOption speedOption({"s", "speed"}
								   ...
								   , "speed", "0");
	QCommandLineOption exitOnSuccessOption({"x", "exit"}
	...
	parser.addOption(exitOnSuccessOption);
	...
	const bool exitOnSuccessMode = parser.isSet(exitOnSuccessOption);
	...
	if (!runner->interpret(qrsFile, backgroundMode, speedFactor, exitOnSuccessMode, showConsoleMode)) {
		return 2;
	}
