...
make -j2 qmake_all 1>>build.log 2>&1
make -j2 all 1>>build.log 2>&1
ls bin/$CONFIG
{ export QT_QPA_PLATFORM=minimal ;
     export ASAN_OPTIONS=detect_stack_use_after_return=1:fast_unwind_on_malloc=0:use_sigaltstack=0
...
df -h .
buildScripts/travis/checkStatus.sh
